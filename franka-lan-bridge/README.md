...
- 服务端速度、单步位移、XYZ 工作空间和 dynamics factor 二次限幅；连续速度会按当前末端姿态换算到 Base 坐标，并预测一个速度看门狗周期内的位置，当前或预测位置越界时由机器人侧立即停止；
...
- 连续速度每次下发前都会检查当前位置和短时预测位置；越过 `workspace_min_m` / `workspace_max_m` 时 fail closed，状态流的 `bridge.velocity_workspace_blocked`、`workspace_guard_stop_count` 和 `last_workspace_guard_reason` 可用于诊断；
